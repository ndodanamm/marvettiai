...
export const WhatsAppPanel: React.FC<Props> = ({ message }) => {
  ...
  return (
    <div className="glass-panel rounded-[3.5rem] overflow-hidden border border-white/5 shadow-2xl animate-in slide-in-from-bottom-8 duration-700">
      <div className="p-8 md:p-10 border-b border-white/5 bg-white/5 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-black text-white flex items-center gap-3 tracking-tight uppercase text-lg">
        ...
        <div className="hidden sm:flex gap-1.5 items-center px-4 py-2 rounded-full bg-slate-950 border border-white/5">
      ...
      <div className="p-8 md:p-12 space-y-8">
        <div className="space-y-4">
           ...
           {message ? (
             <div className="bg-slate-950/50 p-8 md:p-10 rounded-[2.5rem] text-sm md:text-base border border-white/5 text-slate-300 leading-relaxed font-medium shadow-inner">
               {message}
             </div>
           ) : (
             <div className="bg-slate-950/30 p-16 rounded-[2.5rem] text-center border border-dashed border-white/10 flex flex-col items-center gap-4">
                <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center text-3xl grayscale opacity-20">📡</div>
                <p className="text-slate-600 text-[10px] font-black uppercase tracking-[0.3em] italic">Awaiting Module Data for Uplink Generation</p>
             </div>
           )}
        </div>
