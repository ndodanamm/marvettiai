...
const Modal: React.FC<{ isOpen: boolean; onClose: () => void; title: string; children: React.ReactNode }> = ({ isOpen, onClose, title, children }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/40 dark:bg-slate-950/90 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[3rem] p-10 shadow-2xl relative border border-black/5 dark:border-white/10 animate-in zoom-in-95 duration-200">
        <button onClick={onClose} className="absolute top-8 right-8 text-slate-400 hover:text-[#EC1B23] transition-colors p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/5">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12"/></svg>
        </button>
        <h3 className="text-2xl font-black mb-8 text-slate-900 dark:text-white tracking-tight uppercase">{title}</h3>
        {children}
      </div>
    </div>
  );
...
const ThemeToggle: React.FC<{ theme: 'light' | 'dark'; onToggle: () => void }> = ({ theme, onToggle }) => (
  <button 
    onClick={onToggle}
    className="p-3 rounded-xl glass-button text-slate-500 hover:text-[#EC1B23] transition-all"
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setState(parsed);
        if (parsed.client) setView('dashboard');
      } catch (e) {
  ...
  if (!isLoaded) return <div className="min-h-screen bg-slate-50 dark:bg-[#020617] flex items-center justify-center">
    <div className="w-12 h-12 border-4 border-[#EC1B23]/20 border-t-[#EC1B23] rounded-full animate-spin"></div>
  </div>;

  const renderLandingPage = () => (
    <div className="min-h-screen text-slate-900 dark:text-slate-200 bg-grid animate-in fade-in duration-1000">
      <nav className="fixed top-0 z-50 w-full px-6 py-8 flex items-center justify-between pointer-events-none">
        <div className="max-w-7xl mx-auto w-full flex items-center justify-between pointer-events-auto">
          <div className="flex items-center gap-3 glass-panel px-5 py-2.5 rounded-2xl cursor-pointer hover:scale-105 transition-all" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            ...
            <h1 className="text-xl font-black tracking-tighter uppercase">MARVETTI<span className="text-[#EC1B23]">.AI</span></h1>
          </div>
          
          <div className="flex items-center gap-4">
            <ThemeToggle theme={state.theme} onToggle={toggleTheme} />
            <button onClick={() => setIsQuoteOpen(true)} className="hidden sm:block px-6 py-3 glass-button rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-[#EC1B23]">Get Quote</button>
      ...
      <header className="relative px-6 pt-48 pb-56 flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10">
          ...
          <h2 className="text-6xl md:text-[9rem] font-black leading-[0.85] mb-12 tracking-tighter text-slate-900 dark:text-white uppercase">
            Start your<br/>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#EC1B23] via-red-500 to-[#770E13]">Business fast.</span>
          </h2>
          <p className="max-w-2xl mx-auto text-xl text-slate-600 dark:text-slate-400 mb-16 font-medium leading-relaxed">
          ...
          <div className="flex flex-col md:flex-row gap-8 justify-center items-center">
            ...
            <button onClick={() => setIsQuoteOpen(true)} className="w-full md:w-auto px-16 py-6 glass-button text-slate-900 dark:text-white text-xl font-black rounded-[2rem]">Calculate Roadmap</button>
          </div>
        </div>
      </header>

      <section className="py-40 px-6 border-y border-black/5 dark:border-white/5">
         <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
            {[
              ...
              <div key={i} className="glass-panel p-12 rounded-[3rem] border border-black/5 dark:border-white/5 hover:border-[#EC1B23]/30 transition-all group hover:-translate-y-2 duration-500">
                 <div className="text-5xl mb-10 group-hover:scale-110 transition-transform origin-left">{c.icon}</div>
                 <h4 className="text-3xl font-black mb-6 text-slate-900 dark:text-white uppercase tracking-tight">{c.t}</h4>
                 <p className="text-slate-600 dark:text-slate-400 text-lg leading-relaxed font-medium">{c.d}</p>
              </div>
            ))}
         </div>
      </section>
      ...
      <footer className="py-32 px-6 flex flex-col items-center text-center">
         ...
         <h2 className="text-4xl md:text-6xl font-black text-slate-900 dark:text-white tracking-tighter mb-12 uppercase">Own your future.</h2>
         <p className="text-slate-400 dark:text-slate-500 font-bold text-sm tracking-[0.3em] uppercase mb-16">© 2025 MARVETTI.AI • SIMPLE BUSINESS SOLUTIONS</p>
      </footer>

      <Modal isOpen={isSignUpOpen} onClose={() => setIsSignUpOpen(false)} title="Dashboard Enrollment">
         <form onSubmit={handleSignUpSubmit} className="space-y-5">
            <div className="grid grid-cols-2 gap-4">
               <input required placeholder="FIRST NAME" className="w-full p-5 bg-white dark:bg-slate-950 border border-black/10 dark:border-white/10 rounded-2xl text-slate-900 dark:text-white font-bold outline-none focus:border-[#EC1B23] text-sm" value={formData.firstName} onChange={e => setFormData({...formData, firstName: e.target.value})} />
               <input required placeholder="SURNAME" className="w-full p-5 bg-white dark:bg-slate-950 border border-black/10 dark:border-white/10 rounded-2xl text-slate-900 dark:text-white font-bold outline-none focus:border-[#EC1B23] text-sm" value={formData.lastName} onChange={e => setFormData({...formData, lastName: e.target.value})} />
            </div>
            <input required type="email" placeholder="EMAIL ADDRESS" className="w-full p-5 bg-white dark:bg-slate-950 border border-black/10 dark:border-white/10 rounded-2xl text-slate-900 dark:text-white font-bold outline-none focus:border-[#EC1B23] text-sm" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
            <input required type="tel" placeholder="WHATSAPP NUMBER" className="w-full p-5 bg-white dark:bg-slate-950 border border-black/10 dark:border-white/10 rounded-2xl text-slate-900 dark:text-white font-bold outline-none focus:border-[#EC1B23] text-sm" value={formData.cell} onChange={e => setFormData({...formData, cell: e.target.value})} />
      ...
      <Modal isOpen={isQuoteOpen} onClose={() => setIsQuoteOpen(false)} title="Strategic Roadmap">
         <div className="space-y-6">
            <select className="w-full p-5 bg-white dark:bg-slate-950 border border-black/10 dark:border-white/10 rounded-2xl text-slate-900 dark:text-white font-bold appearance-none outline-none focus:border-[#EC1B23] text-sm">
            ...
            <div className="bg-red-600/5 p-8 rounded-3xl border border-red-600/10">
               <p className="text-xs text-slate-500 font-medium leading-relaxed">Most registrations start at R495. Sign up now to see your full success path.</p>
            </div>
            <button onClick={() => setIsSignUpOpen(true)} className="w-full py-6 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black rounded-2xl hover:scale-105 transition-all uppercase tracking-widest text-[10px]">Start Success Path</button>
         </div>
      </Modal>
    </div>
  );
  ...
  const renderDashboard = () => {
    ...
    return (
      <div className="h-screen flex flex-col animate-in fade-in duration-700 bg-slate-50 dark:bg-[#020617]">
        <header className="h-20 glass-panel border-b border-black/5 dark:border-white/5 flex items-center justify-between px-6 md:px-10 shrink-0 z-50">
          <div className="flex items-center gap-6 md:gap-10">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="lg:hidden p-2 text-slate-400 hover:text-[#EC1B23] transition-colors">
            ...
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => setView('landing')}>
               ...
               <h1 className="text-xl font-black tracking-tighter hidden sm:block uppercase text-slate-900 dark:text-white">MARVETTI<span className="text-[#EC1B23]">.AI</span></h1>
            </div>
            <div className="h-8 w-px bg-black/5 dark:bg-white/10 hidden md:block"></div>
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-800 dark:to-slate-950 flex items-center justify-center text-slate-900 dark:text-white font-black text-sm border border-black/5 dark:border-white/5 shadow-inner">
                {state.client?.firstName[0]}
              </div>
              <div className="hidden lg:block">
                <p className="text-xs font-black uppercase text-slate-900 dark:text-white tracking-tight leading-none">{state.client?.firstName} {state.client?.lastName}</p>
          ...
          <div className="flex items-center gap-6">
             ...
             <div className="hidden xl:flex gap-2 items-center">
                {Array.from({length: 12}).map((_, i) => (
                  <div key={i} className={`w-1.5 h-1.5 rounded-full transition-all duration-700 ${i + 1 <= state.currentStage ? 'bg-[#EC1B23] shadow-[0_0_8px_#EC1B23]' : 'bg-black/10 dark:bg-white/10'}`}></div>
                ))}
             </div>
             <button onClick={resetSession} className="px-5 py-2.5 glass-button rounded-xl text-[9px] font-black uppercase tracking-widest hover:text-[#EC1B23] text-slate-500 dark:text-slate-400">Reset Session</button>
          </div>
        </header>
        ...
        <div className="flex-1 flex overflow-hidden relative">
          <aside className={`fixed inset-0 z-40 lg:relative lg:z-0 lg:flex w-80 glass-panel border-r-0 flex-col transition-transform duration-500 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
            <div className="h-full bg-white dark:bg-slate-950 lg:bg-transparent flex flex-col">
              <div className="p-8 lg:mt-0 mt-20">
                ...
                <div className="h-2 w-full bg-slate-200 dark:bg-slate-950 rounded-full overflow-hidden border border-black/5 dark:border-white/5">
                ...
                {Object.values(state.stages).map((s: StageInfo) => (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-2xl flex items-center justify-between transition-all group relative overflow-hidden ${
                      ...
                      : (s.isUnlocked ? 'hover:bg-black/5 dark:hover:bg-white/5 text-slate-500 dark:text-slate-400' : 'opacity-10 grayscale cursor-not-allowed')
                    }`}
                  >
              ...
              <div className="p-6">
                <button onClick={() => setState(prev => ({...prev, isAdminMode: !prev.isAdminMode}))} className="w-full p-4 glass-button text-[9px] font-black uppercase tracking-widest rounded-2xl text-slate-500 border-black/5 dark:border-white/5">
          ...
          <main className="flex-1 flex flex-col overflow-hidden relative">
             ...
             <div className="flex-1 overflow-y-auto p-6 md:p-12 lg:p-20 relative z-10 custom-scrollbar" ref={dashboardTopRef}>
                <div className="max-w-5xl mx-auto space-y-12 md:space-y-16">
                   <div className="space-y-6">
                      <div className="inline-flex items-center gap-3 px-4 py-1.5 glass-panel text-[10px] font-black text-[#EC1B23] rounded-full uppercase tracking-widest border border-black/5 dark:border-white/5">
                         Active Module • Stage {state.currentStage}
                      </div>
                      <h2 className="text-4xl md:text-7xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-[0.9]">{currentStageInfo.name.split('. ')[1]}</h2>
                      <p className="text-lg md:text-xl text-slate-600 dark:text-slate-400 font-medium max-w-2xl leading-relaxed">{currentStageInfo.description}</p>
                   </div>
                   ...
                   {state.isAdminMode && (
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in zoom-in-95 duration-500">
                         <div className="p-6 bg-amber-600/10 border border-amber-600/20 rounded-3xl">
                            ...
                            <p className="text-slate-900 dark:text-white font-black text-xs">MEDIUM</p>
                         </div>
                         <div className="p-6 bg-emerald-600/10 border border-emerald-600/20 rounded-3xl">
                            ...
                            <p className="text-slate-900 dark:text-white font-black text-xs">ON TRACK (14m)</p>
                         </div>
                         <div className="p-6 bg-blue-600/10 border border-blue-600/20 rounded-3xl">
                            ...
                            <p className="text-slate-900 dark:text-white font-black text-xs">REGULATED NICHE</p>
                         </div>
                      </div>
                   )}

                   <div className="glass-panel rounded-[3.5rem] p-8 md:p-16 shadow-2xl relative overflow-hidden border border-black/5 dark:border-white/5">
                   ...
                   {stageSummary && (
                     <div className="animate-in slide-in-from-bottom-12 duration-700">
                       <div className="flex items-center gap-5 mb-8">
                          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#EC1B23] to-[#770E13] flex items-center justify-center shadow-2xl shadow-red-500/30 border border-black/5 dark:border-white/10">
                          ...
                          <div className="space-y-1">
                             <h4 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Status Report</h4>
                             <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Protocol Sync Complete</p>
                          </div>
                       </div>
                       <div 
                         className="prose prose-slate dark:prose-invert max-w-none glass-panel p-8 md:p-16 rounded-[3.5rem] shadow-2xl font-medium leading-relaxed border border-black/5 dark:border-white/5 text-slate-600 dark:text-slate-400"
